...
class InstallGUI:
	...
	def set_style(self):
		...
		provider.load_from_path(running_folder + "/Theme/Windows7/gtk-3.0/gtk.css")
	...
	def get_sizes(self):
			...
			self.icons_size_kb = self.folder_size(running_folder+"/Icons/Windows7")
	...
	def next_clicked(self, button):
		...
		if next_button.get_label() == "Finish":
			print("Install Completed! Enjoy Windows7!")
			Gtk.main_quit()
			return False
	...
	def install_chicago95(self):
		...
		customizations = "\tChange Terminal Colors:\t{}\n\tSet Bash Prompt:\t{}\n\tSet zsh promt/theme:\t{}".format(self.terminal_colors, self.bash, self.zsh)
		...
		if self.install_theme:
			...
			self.copy_files.update(self.get_files(running_folder+"/Theme/Windows7/", os.path.expanduser("~/.themes"), "Theme"))
		...
		if self.install_icons:
			...
			self.copy_files.update(self.get_files(running_folder+"/Icons/Windows7/", os.path.expanduser("~/.icons"), "Icons"))
		...
		if self.install_sounds:
			...
			self.copy_files.update(self.get_files(running_folder+"/sounds/Windows7/", os.path.expanduser("~/.local/share/sounds"), "sounds"))
		...
		if self.terminal_colors:
			...
			self.copy_files[running_folder+"/Extras/Windows7.theme"] = os.path.expanduser("~/.local/share/xfce4/terminal/colorschemes")
			...
			if os.path.exists(os.path.expanduser("~/.config/xfce4/terminal/terminalrc")):
				shutil.copyfile(os.path.expanduser("~/.config/xfce4/terminal/terminalrc"),os.path.expanduser("~/.config/xfce4/terminal/backup.terminalrc.Windows7"))
		...
		for from_file in self.copy_files:
			...
			if isinstance(self.copy_files[from_file], str):
				...
				if from_file == "install_theme" and self.copy_files["install_theme"]:
					...
					self.xfconf_query('xsettings', '/Net/ThemeName', "Windows7")
					self.xfconf_query('xfce4-notifyd', '/theme', "Windows7")
					self.xfconf_query('xfce4-notifyd', '/initial-opacity', "1.000000")
					self.xfconf_query('xfwm4', '/general/theme', "Windows7")
					...
					self.xfconf_query('xsettings', '/Net/IconThemeName', "Windows7")
					...
					self.xfconf_query('xsettings', '/Gtk/CursorThemeName', "Windows7 Standard Cursors")
					self.change_component_label()
					...
					self.xfconf_query('xsettings', '/Net/SoundThemeName', "Windows7")
					self.change_component_label()
          ...
          # LOL this is a lie we don't have to do anything
					...
					if os.path.exists(os.path.expanduser("~/.themes/Windows7/gtk-3.24/apps/thunar.css")):
						print("Enabling authenticity in Thunar")
						shutil.move( os.path.expanduser("~/.themes/Windows7/gtk-3.24/apps/thunar.css"),os.path.expanduser("~/.themes/Windows7/gtk-3.24/apps/thunar.css.bak") )
						fileh = open(os.path.expanduser("~/.themes/Windows7/gtk-3.24/apps/thunar.css.bak"),"r")
						nfileh = open(os.path.expanduser("~/.themes/Windows7/gtk-3.24/apps/thunar.css"),"w")
						next_line = False
						for line in fileh:
							if next_line:
								if "/*" in line:
						...
						print("Windows7 theme not installed, cannot modify thunar")
					self.change_component_label()
					...
					if os.path.exists(os.path.expanduser("~/.bashrc")):
						shutil.copyfile(os.path.expanduser("~/.bashrc"),os.path.expanduser("~/.config/xfce4/terminal/backup.bashrc.Windows7"))
					...
					if os.path.exists(os.path.expanduser("~/.oh-my-zsh")):
						...
						shutil.copyfile(running_folder+"/Extras/Windows7.zsh-theme", os.path.expanduser("~/.oh-my-zsh/themes/Windows7.zsh-theme"))
						shutil.move( os.path.expanduser("~/.zshrc"),os.path.expanduser("~/.backup.zshrc.Windows7") )
						fileh = open(os.path.expanduser("~/.backup.zshrc.Windows7"),"r")
						nfileh = open(os.path.expanduser("~/.zshrc"),"w")
						for line in fileh:
							if "ZSH_THEME" in line:
								line = "ZSH_THEME=Windows7"
							nfileh.write(line)
						fileh.close()
						nfileh.close()
					else:
	...
	def xfconf_query(self, channel, prop, new_value):
		...
		try:
			...
			print("Warning: xfconf-query not installed, cannot auto-enable theme. Use your distros theme management to install Windows7")
			return
